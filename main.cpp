...
{
    int food;//此screen_information[x][y]处是否有食物有的话food为1
    int star;//此screen_information[x][y]处是否有星星的话值为1
    int barrier;//此screen_information[x][y]处是否有障碍物有的话值为1
...
void qp();//清除屏幕
int input();//绘制选关页面 
void load_file(int id)//从配置文件导入关卡设置 
{
	n=4;//n用来记录蛇身长度,初始为3节
    if(HP!=-1) HP=6;//记录蛇的生命值,初始化为6
    p[0].x=6;                    //
    p[0].y=11;                   //
    p[0].direction=RIGHT;        //
    p[1].x=4;                   //
    p[1].y=11;                  //     初始化蛇所在位置和移动方向   
    p[1].direction=RIGHT;       //
    p[2].x=2;                   //
    p[2].y=11;                  //
    p[2].direction=RIGHT;      //
...
inline void start()//绘制墙/绘制启动画面以及隔墙
{
	static int tt1,wait;
	wait=1;
...
int input()
{
	...
	if(passed==n)
	{
		...
		SetColor(14);
		printf("您已通过所有关卡\n");
		printf("建议从http://download.csdn.net/download/alphago_zero/10223607下载关卡生成器\n");
		printf("打开链接？[Y/N] ");
		char c=getch();
		if(c=='y'||c=='Y')
		{
...
inline void guanka()//用来选择关卡并根据关卡设置蛇的移动速度
{
	qp();
    static char t; 
    while(1)
    {
        ...
        if(guan<7&&guan>0) break;//判断速度是否溢出
        gotoxy(15,5);
        cout<<"输入错误！";
        Sleep(800);
        gotoxy(15,5);
    ...
    switch(guan)
    {
        case 1:{T=600;break;}//
        case 2:{T=400;break;}//
        case 3:{T=200;break;}//    根据速度来设定蛇的移动速度
        case 4:{T=150;break;}//
        case 5:{T=100;break;}//
        case 6:{T=60;break;}//
    }
...
inline void data()//用来记录和判断游戏的各种状态数据
{
    gotoxy(66,2);
    cout<<"        "; 
    gotoxy(66,2);
...
inline void qp()//用来清除屏幕
{
    ...
    for(int i=0;i<22;i++)
    	for(int j=0;j<26;j++)
    	{
			screen_information[j][i].food=0;//将食物清空
			screen_information[j][i].barrier=0;//将障碍物清除
			screen_information[j][i].star=0;//将星星清除 
		}
}

inline void show()//用来随机产生障碍物以及食物和生命药水以及用来判断游戏的各种参数
{
    static int a,b,e,f,i; //a，b用来表示小星星的坐标   c，d代表障碍物坐标
    if(screen_information[x][y].food==0)//判断食物是不是被吃掉
    { 
        while(1)
        {                                                                                                  
            x=random()%26;//产生随机横坐标
            y=random()%22;//产生随机纵坐标
            if(screen_information[x][y].barrier==0&&screen_information[x][y].star==0) break;//当此处无其他元素是才生效 
        }
        screen_information[x][y].food=1;//随机出现食物
    ...
    if(t1/20>0&&t1%12==0&&t1>t3&&screen_information[(p[0].x-1)/2][p[0].y-1].food==0&&screen_information[(p[0].x-1)/2][p[0].y-1].star==0)
    {
        ...
        {                                                                                                  
            e=random()%26;//产生随机横坐标
            f=random()%22;//产生随机纵坐标
            if(screen_information[e][f].food==0&&screen_information[e][f].star==0) break;//当此处无其他元素是才生效 
        }
        gotoxy(2*(e+1),f+1);//定位到障碍物出现的位置
        screen_information[e][f].barrier=1;//随机出现障碍物
        c(12);//调成黄色
        cout<<"▇";//打印出障碍物
        t3=t1;//以免产生多个障碍物
        if(HP<7)
        {
          gotoxy(18,24);
          c(15);//调成白色
          cout<<"温馨提示：在选速度的时候输入0可以开启作弊模式";
        }
    }
    if(t1/25>0&&t1%15==0&&t1>t3&&screen_information[(p[0].x-1)/2][p[0].y-1].food==0&&screen_information[(p[0].x-1)/2][p[0].y-1].barrier==0)//减少星星出现的几率 
    {
        while(1)
        {                                                                                                  
            a=random()%26;//产生随机横坐标
            b=random()%22;//产生随机纵坐标
            if(screen_information[a][b].barrier==0&&screen_information[a][b].food==0) break;//当此处无其他元素是才生效 
        }
        screen_information[a][b].star=1;//随机出现小星星（吃到星星长度减1）
    ...
    for(i=0;i<n;i++)
    {
        if(screen_information[(p[i].x-1)/2][p[i].y-1].food==1)//判断蛇是否吃到食物
        {
            ++n;//让蛇长度加1
            food++;//将食物数加1
            screen_information[(p[i].x-1)/2][p[i].y-1].food=0;//让食物标示归零
            break;
        }
    }
    if(screen_information[(p[0].x-1)/2][p[0].y-1].star==1)//判断蛇是否吃到星星
    {
        screen_information[(p[0].x-1)/2][p[0].y-1].star=0;//让星星标示归零
        if(HP<6)
           ++HP;//将生命值加1
    }
    t1=time()-t2;//刷新游戏运行时间
}
inline void key()//处理用户操作
{
	static int ch;
    if(kbhit())//判断是否按键
    {              
...
inline int game()
{
    ...
    while(1)
    {
        ...
        if(screen_information[(p[0].x-1)/2][p[0].y-1].barrier==1)//当蛇障碍物时
        {
            cout<<"\a";
            --HP;//将生命值减一
